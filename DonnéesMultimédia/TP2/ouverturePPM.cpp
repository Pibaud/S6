...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < nH-1; i++) {
    for (int j = 1; j < nW-1; j++) {
      for (int k = 0; k < 3; k++) {
        int maxVal = std::max({
            ImgTemp[((i-1)*nW+(j-1))*3+k], ImgTemp[((i-1)*nW+j)*3+k], ImgTemp[((i-1)*nW+(j+1))*3+k],
            ImgTemp[(i*nW+(j-1))*3+k], ImgTemp[(i*nW+j)*3+k], ImgTemp[(i*nW+(j+1))*3+k],
            ImgTemp[((i+1)*nW+(j-1))*3+k], ImgTemp[((i+1)*nW+j)*3+k], ImgTemp[((i+1)*nW+(j+1))*3+k]
        });
